...
SemaphoreHandle_t xSemaphore_r; //cria variavel do semaforo
...
void btn_1_task(void *p) {
  ...
  while (true) {
    if (!gpio_get(BTN_PIN_R)) {
      ...
      xSemaphoreGive(xSemaphore_r); //libera semaforo
    }
  }
}
...
int main() {
  ...
  xSemaphore_r = xSemaphoreCreateBinary(); //inicia semaforo
